...
class WorkoutService:
    ...
    def normalise_datetime_to_utc(self, date_time_value: datetime) -> datetime:
        ...
        """
        if (
            date_time_value.tzinfo is None
            or date_time_value.tzinfo.utcoffset(date_time_value) is None
        ):
            return date_time_value.replace(tzinfo=timezone.utc)
        return date_time_value.astimezone(timezone.utc)

    def calculate_estimated_one_repetition_maximum(
        self, weight_in_kilograms: float, repetitions: int
    ) -> float:
        """
    ...
    def retrieve_specific_workout(
        ...
        return self.workout_repository.retrieve_workout_by_identifier(
            identifier=identifier, user_identifier=user_identifier
        )

    def remove_workout_session(
        self, identifier: str, user_identifier: Optional[str] = None
    ) -> bool:
        """
        ...
        for workout in workouts_sorted_desc:
            for exercise in workout.exercises:
                exercise_definition_identifier = (
                    exercise.exercise_definition_identifier or ""
                ).strip()
                if (
                    not exercise_definition_identifier
                    or exercise_definition_identifier in last_used_values
                ):
                    continue
                ...
                last_used_values[exercise_definition_identifier] = {
                    ...
                    "workout_date": self.normalise_datetime_to_utc(
    ...
    def build_profile_summary(self, user_identifier: str) -> dict[str, object]:
        ...
        total_sets = sum(
            len(exercise.sets) for workout in workouts for exercise in workout.exercises
        )
    ...
    def calculate_best_estimated_one_repetition_maximum(
        self, workout_document: WorkoutDocument
    ) -> float:
        """
    ...
    def calculate_average_workout_rpe(self, workout_document: WorkoutDocument) -> float:
        ...
        """
        workout_sets = [
            workout_set for exercise in workout_document.exercises for workout_set in exercise.sets
        ]
        if not workout_sets:
            return 0.0
        recorded_rpe_values = [
    ...
    def calculate_best_estimated_one_repetition_maximum_for_exercise(
        self, workout_document: WorkoutDocument, exercise_name: str
    ) -> float:
        """
    ...
    def calculate_training_streak_weeks(self, workouts: List[WorkoutDocument]) -> int:
        ...
        unique_weeks = sorted(
            {
                (
                    workout.date_of_workout.isocalendar().year,
                    workout.date_of_workout.isocalendar().week,
                )
                for workout in workouts
            },
            reverse=True,
    ...
    def build_personal_records(
        self, workouts: List[WorkoutDocument], limit: int = 5
        ...
        for workout in workouts:
            for exercise in workout.exercises:
                best_estimated_maximum = (
                    self.calculate_best_estimated_one_repetition_maximum_for_exercise(
                        workout_document=workout, exercise_name=exercise.exercise_name
                    )
                )
                ...
                if (
                    ...
                    or best_estimated_maximum > existing_record["estimated_one_rep_maximum"]
                ):
    ...
    def filter_workouts_by_range(
        self, workouts: List[WorkoutDocument], range_days: Optional[int]
    ) -> List[WorkoutDocument]:
        """
        ...
        if range_days < 0:
            logger.warning(
                "Invalid range_days value provided to filter_workouts_by_range: %s", range_days
            )
    ...
    def build_dashboard_analytics(
        ...
        available_exercise_names = sorted(
            {exercise.exercise_name for workout in all_workouts for exercise in workout.exercises}
        )
        ...
        total_volume = round(
            sum(self.calculate_total_workout_volume(workout) for workout in filtered_workouts), 2
        )
        total_sets = sum(self.calculate_total_sets(workout) for workout in filtered_workouts)
        total_repetitions = sum(
            self.calculate_total_repetitions(workout) for workout in filtered_workouts
        )
        total_exercises = sum(len(workout.exercises) for workout in filtered_workouts)
        workout_rpe_averages = [
            self.calculate_average_workout_rpe(workout) for workout in filtered_workouts
        ]
        recorded_workout_rpe_averages = [average for average in workout_rpe_averages if average > 0]
        average_session_rpe = (
            round(sum(recorded_workout_rpe_averages) / len(recorded_workout_rpe_averages), 2)
            if recorded_workout_rpe_averages
            else 0.0
        )
        volume_labels = [
            workout.date_of_workout.strftime("%Y-%m-%d") for workout in filtered_workouts
        ]
        volume_values = [
            self.calculate_total_workout_volume(workout) for workout in filtered_workouts
        ]
        if selected_exercise_name:
            one_rep_workouts = [
                workout
                ...
                if any(
                    exercise.exercise_name == selected_exercise_name
                    for exercise in workout.exercises
                )
            ]
            one_rep_max_labels = [
                workout.date_of_workout.strftime("%Y-%m-%d") for workout in one_rep_workouts
            ]
            one_rep_max_values = [
                self.calculate_best_estimated_one_repetition_maximum_for_exercise(
                    workout, selected_exercise_name
                )
        ...
        for workout in filtered_workouts:
            ...
            for exercise in workout.exercises:
                top_exercise_volume_counter[exercise.exercise_name] += (
                    self.calculate_exercise_volume(exercise)
                )
        sorted_weekly_labels = sorted(weekly_frequency_counter.keys())
        weekly_frequency_values = [
            weekly_frequency_counter[label] for label in sorted_weekly_labels
        ]
        ...
        average_workout_volume = (
            round(total_volume / len(filtered_workouts), 2) if filtered_workouts else 0.0
        )
