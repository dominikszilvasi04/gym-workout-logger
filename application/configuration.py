...
class ApplicationConfiguration:
    ...
    SECRET_KEY: Optional[str] = os.environ.get(
        "APPLICATION_SECRET_KEY", "default_development_secret_key"
    )
    ...
    SESSION_COOKIE_SECURE: bool = (
        os.environ.get("APPLICATION_SESSION_COOKIE_SECURE", "false").lower() == "true"
    )
    CSRF_PROTECTION_ENABLED: bool = (
        os.environ.get("APPLICATION_CSRF_PROTECTION_ENABLED", "true").lower() == "true"
    )
    ...
    EXERCISE_REQUEST_RECIPIENT_EMAIL: Optional[str] = os.environ.get(
        "APPLICATION_EXERCISE_REQUEST_RECIPIENT_EMAIL"
    )
    ...
    GOOGLE_OAUTH_ENABLED: bool = (
        os.environ.get("APPLICATION_GOOGLE_OAUTH_ENABLED", "false").lower() == "true"
    )
...
class TestingConfiguration(ApplicationConfiguration):
    ...
    APPLICATION_CACHE_DEFAULT_TIMEOUT_SECONDS: int = 1
...
class ProductionConfiguration(ApplicationConfiguration):
    ...
    DEBUG: bool = False
    SESSION_COOKIE_SECURE: bool = (
        os.environ.get("APPLICATION_SESSION_COOKIE_SECURE", "true").lower() == "true"
    )
