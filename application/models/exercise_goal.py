...
class ExerciseGoalDocument(BaseModel):
    ...
    exercise_name: str = Field(
        ..., min_length=1, max_length=120, description="Exercise name the goal targets."
    )
    exercise_definition_identifier: Optional[str] = Field(
        default=None, description="Optional canonical exercise identifier."
    )
    target_weight_in_kilograms: float = Field(
        ..., gt=0, description="Goal target weight in kilograms."
    )
    target_repetitions: int = Field(..., gt=0, description="Goal target repetitions.")
    target_date: Optional[date] = Field(
        default=None, description="Optional target date for the goal."
    )
