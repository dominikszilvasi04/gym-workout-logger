...
class WorkoutTemplateDocument(BaseModel):
    ...
    template_name: str = Field(
        ..., min_length=1, max_length=80, description="Human readable template name."
    )
    target_muscle_groups: list[str] = Field(
        default_factory=list, description="Primary muscle groups for the template."
    )
    exercises: list[ExerciseLog] = Field(
        default_factory=list, description="Preconfigured exercises and set defaults."
    )
