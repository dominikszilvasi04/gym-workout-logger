...
def retrieve_exercises_endpoint() -> tuple[Response, int]:
    ...
    standardised_exercises = (
        application_exercise_definition_service.retrieve_all_standardised_exercises()
    )
...
def request_new_exercise_endpoint() -> tuple[Response, int]:
    ...
    requester_email = (
        str(request_payload.get("requester_email", "")).strip()
        or str(session.get("user_email", "")).strip()
    )
    ...
    request_notes = (
        str(request_payload.get("notes", "")).strip()
        or str(request_payload.get("additional_notes", "")).strip()
    )
