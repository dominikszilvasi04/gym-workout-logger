...
def view_dashboard() -> tuple[str, int] | str | Response:
    ...
    try:
        ...
        if user_identifier:
            workout_templates = application_workout_template_service.retrieve_templates(
                user_identifier=user_identifier
            )
    except RuntimeError:
    ...
    return render_template(
        "dashboard.html", workouts=workout_history, workout_templates=workout_templates
    )
...
def retrieve_dashboard_analytics_endpoint() -> tuple[Response, int]:
    ...
    try:
        ...
        logger.exception(
            "Dashboard analytics failed because the database client is not initialised."
        )
...
def retrieve_dashboard_initial_data_endpoint() -> tuple[Response, int]:
    ...
    try:
        ...
        templates = application_workout_template_service.retrieve_templates(
            user_identifier=user_identifier
        )
        exercises = application_exercise_definition_service.retrieve_available_exercises()
        goals = application_exercise_goal_service.build_goal_progress_payload(
            user_identifier=user_identifier
        )
    except RuntimeError:
        logger.exception(
            "Dashboard initial data request failed because the database client is not initialised."
        )
        return jsonify({"error": "Database unavailable."}), 503
...
def retrieve_workouts_endpoint() -> tuple[Response, int]:
    ...
    sort_field = (
        request.args.get("sort", default="date_of_workout", type=str) or "date_of_workout"
    ).strip()
    ...
    try:
        ...
        logger.exception(
            "Workout list request failed because the database client is not initialised."
        )
        return jsonify({"error": "Database unavailable."}), 503
...
def create_workout_endpoint() -> tuple[Response, int]:
    ...
    try:
        ...
        return jsonify(
            {"error": "Data validation failed.", "details": validation_error.errors()}
        ), 422
    inserted_identifier = application_workout_service.record_new_workout_session(
        workout_document=workout_document
    )
    logger.info("Workout created successfully with identifier=%s", inserted_identifier)
    return jsonify(
        {"message": "Workout successfully recorded.", "identifier": inserted_identifier}
    ), 201
...
def retrieve_last_workout_endpoint() -> tuple[Response, int]:
    ...
    user_identifier = get_authenticated_user_identifier()
    try:
        most_recent_workout = application_workout_service.retrieve_most_recent_workout(
            user_identifier=user_identifier
        )
    except RuntimeError:
        logger.exception(
            "Last workout request failed because the database client is not initialised."
        )
        return jsonify({"error": "Database unavailable."}), 503
...
def retrieve_last_used_values_endpoint() -> tuple[Response, int]:
    ...
    user_identifier = get_authenticated_user_identifier()
    try:
        last_used_values = application_workout_service.build_last_used_values_map(
            user_identifier=user_identifier
        )
    except RuntimeError:
        logger.exception(
            "Last-used values request failed because the database client is not initialised."
        )
        return jsonify({"error": "Database unavailable."}), 503
...
def retrieve_workout_detail_endpoint(identifier: str) -> tuple[Response, int]:
    ...
    try:
        ...
        logger.exception(
            "Workout detail request failed because the database client is not initialised."
        )
        return jsonify({"error": "Database unavailable."}), 503
    if not workout:
        logger.warning("Workout detail not found for identifier=%s", identifier)
...
def retrieve_workout_templates_endpoint() -> tuple[Response, int]:
    ...
    try:
        ...
        logger.exception(
            "Template list request failed because the database client is not initialised."
        )
        return jsonify({"error": "Database unavailable."}), 503
...
def retrieve_workout_template_by_identifier_endpoint(identifier: str) -> tuple[Response, int]:
    ...
    user_identifier = get_authenticated_user_identifier()
    try:
        template = application_workout_template_service.retrieve_template(
            identifier=identifier, user_identifier=user_identifier
        )
    except RuntimeError:
        logger.exception(
            "Template detail request failed because the database client is not initialised."
        )
        return jsonify({"error": "Database unavailable."}), 503
    if template is None:
...
def retrieve_exercise_goals_endpoint() -> tuple[Response, int]:
    ...
    user_identifier = get_authenticated_user_identifier()
    try:
        goals_payload = application_exercise_goal_service.build_goal_progress_payload(
            user_identifier=user_identifier
        )
    except RuntimeError:
...
def delete_exercise_goal_endpoint(identifier: str) -> tuple[Response, int]:
    ...
    user_identifier = get_authenticated_user_identifier()
    try:
        deleted = application_exercise_goal_service.delete_goal(
            identifier=identifier, user_identifier=user_identifier
        )
    except RuntimeError:
...
def create_workout_template_endpoint() -> tuple[Response, int]:
    ...
    try:
        inserted_identifier = application_workout_template_service.create_template(
            template_document
        )
    except RuntimeError:
...
def delete_workout_template_endpoint(identifier: str) -> tuple[Response, int]:
    ...
    user_identifier = get_authenticated_user_identifier()
    try:
        deleted = application_workout_template_service.delete_template(
            identifier=identifier, user_identifier=user_identifier
        )
    except RuntimeError:
...
def update_workout_endpoint(identifier: str) -> tuple[Response, int]:
    ...
    if request_data is None or not isinstance(request_data, dict):
        logger.warning(
            "Update workout rejected for identifier=%s due to invalid JSON payload.", identifier
        )
        return jsonify({"error": "A valid JSON payload is required."}), 400
    try:
        workout_document = WorkoutDocument(**request_data)
        ...
        success = application_workout_service.modify_workout_session(
            identifier, workout_document, user_identifier=user_identifier
        )
        if success:
